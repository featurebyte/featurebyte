...
#* Testing
test:
	@poetry run coverage run -m pytest -c pyproject.toml --timeout=120 --junitxml=pytest.xml tests featurebyte | tee -a pytest-coverage.txt
	@poetry run coverage-badge -o assets/images/coverage.svg -f    # Write coverage badge
