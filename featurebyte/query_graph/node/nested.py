...
class CalendarViewGraphNodeParameters(BaseViewGraphNodeParameters):
    """GraphNode (type:calendar_view) parameters"""

    type: Literal[GraphNodeType.CALENDAR_VIEW] = GraphNodeType.CALENDAR_VIEW
    ...
    def derive_sdk_code(
        ...
        # construct calendar view sdk statement
        view_var_name = var_name_generator.convert_to_variable_name(
            variable_name_prefix="calendar_view", node_name=node_name
        )
        assert len(input_var_name_expressions) == 1
        table_var_name = input_var_name_expressions[0].var_name_or_expr
        expression = get_object_class_from_function_call(
