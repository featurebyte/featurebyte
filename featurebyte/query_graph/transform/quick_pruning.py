...
class QuickGraphStructurePruningTransformer(BaseGraphTransformer[GraphNodeNameMap, QuickGraphPruningGlobalState]):
    ...
    @classmethod
    def _extract_node_names_to_keep(cls, graph: QueryGraphModel, target_node_names: List[str]) -> Set[str]:
        node_names_to_keep: Set[str] = set()
        for target_node_name in target_node_names:
            for node in graph.iterate_nodes(target_node=graph.get_node_by_name(target_node_name), node_type=None):
    ...
    def _transform(self, global_state: QuickGraphPruningGlobalState) -> Any:
        for node_name in self.graph.sorted_node_names:
            if node_name in global_state.node_names_to_keep:
                self._compute(global_state=global_state, node=self.graph.get_node_by_name(node_name))
    ...
    def transform(self, target_node_names: List[str]) -> GraphNodeNameMap:
        ...
        GraphNodeNameMap
        """
        node_names_to_keep = self._extract_node_names_to_keep(graph=self.graph, target_node_names=target_node_names)
