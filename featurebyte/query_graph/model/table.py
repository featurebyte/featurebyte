...
class SnapshotsTableData(BaseTableData):
    """SnapshotsTableData class"""
    ...
    def construct_snapshots_view_graph_node(
        self,
        snapshots_table_node: InputNode,
        drop_column_names: List[str],
        metadata: ViewMetadata,
    ) -> Tuple[GraphNode, List[ColumnInfo]]:
        """
        Construct a graph node & columns info for SnapshotsView of this snapshots table.

        Parameters
        ----------
        snapshots_table_node: InputNode
            Snapshots table node
        drop_column_names: List[str]
            List of column names to drop from the snapshots table
        metadata: ViewMetadata
            Metadata to be added to the graph node
        ...
        """
        view_graph_node, _ = self.construct_view_graph_node(
            graph_node_type=GraphNodeType.SNAPSHOTS_VIEW,
            data_node=snapshots_table_node,
            other_input_nodes=[],
            drop_column_names=drop_column_names,
            metadata=metadata,
        )
...
class CalendarTableData(BaseTableData):
    """CalendarTableData class"""

    type: Literal[TableDataType.CALENDAR_TABLE] = TableDataType.CALENDAR_TABLE
    id: PydanticObjectId = Field(default_factory=ObjectId, alias="_id")
    series_id_column: Optional[StrictStr] = None
    ...
    def construct_input_node(self, feature_store_details: FeatureStoreDetails) -> InputNode:
        return InputNode(
            ...
            parameters={
                ...
                "calendar_datetime_schema": self.calendar_datetime_schema,
                **self._get_common_input_node_parameters(),
            },
        )

    def construct_calendar_view_graph_node(
        self,
        calendar_table_node: InputNode,
        drop_column_names: List[str],
        metadata: ViewMetadata,
    ) -> Tuple[GraphNode, List[ColumnInfo]]:
        """
        Construct a graph node & columns info for CalendarView of this calendar table.
        ...
        calendar_table_node: InputNode
            Calendar table node
        drop_column_names: List[str]
            List of column names to drop from the calendar table
        metadata: ViewMetadata
            Metadata to be added to the graph node
        ...
        """
        view_graph_node, _ = self.construct_view_graph_node(
            graph_node_type=GraphNodeType.CALENDAR_VIEW,
            data_node=calendar_table_node,
            other_input_nodes=[],
            drop_column_names=drop_column_names,
            metadata=metadata,
        )
