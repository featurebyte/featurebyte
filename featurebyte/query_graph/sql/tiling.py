...
class TileSpec:
    ...
    # Only applies for certain tile specs that are vector aggregations.
...
class TilingAggregator(ABC):
    ...
    def construct_numeric_tile_spec(
        ...
        return TileSpec(
            ...
            tile_aggregation_type=agg_func,
        )
...
class NACountAggregator(OrderIndependentAggregator):
    ...
    def tile(self, col: Optional[InputColumn], agg_id: str) -> list[TileSpec]:
        ...
        return [
            self.construct_numeric_tile_spec(
                expressions.Sum(this=col_casted_as_integer), f"value_{agg_id}", AggFunc.NA_COUNT
            )
        ]
...
class StdAggregator(OrderIndependentAggregator):
    ...
    def tile(self, col: Optional[InputColumn], agg_id: str) -> list[TileSpec]:
        ...
        return [
            self.construct_numeric_tile_spec(
                sum_value_squared, f"sum_value_squared_{agg_id}", AggFunc.STD
            ),
            self.construct_numeric_tile_spec(sum_value, f"sum_value_{agg_id}", AggFunc.STD),
            self.construct_numeric_tile_spec(count_value, f"count_value_{agg_id}", AggFunc.STD),
        ]
...
class VectorAvgAggregator(OrderIndependentAggregator):
    ...
    def tile(self, col: Optional[InputColumn], agg_id: str) -> list[TileSpec]:
        ...
        return [
            ...
            self.construct_numeric_tile_spec(
                expressions.Count(this=expressions.Star()), f"count_value_{agg_id}", AggFunc.COUNT
            ),
        ]
    ...
    def tile(self, col: Optional[InputColumn], agg_id: str) -> list[TileSpec]:
        ...
        return [
            TileSpec(
                ...
                tile_aggregation_type=AggFunc.LATEST,
            ),
        ]
