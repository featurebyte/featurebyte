...
class OnlineServingService:  # pylint: disable=too-many-instance-attributes
    ...
    async def get_online_features_by_feast(  # pylint: disable=too-many-locals
        ...
        # Validate deployment can be served using the provided entities
        request_column_names = set(request_data[0].keys())
        ...
        for serving_entity_ids in feature_list.enabled_serving_entity_ids:
            if provided_entity_ids.issuperset(serving_entity_ids):
                break
        else:
            # Provided entities cannot be served, raise an error message with information
            entity_info = EntityInfo(
                required_entities=primary_entities,
        ...
        tic = time.time()
        # TODO: should probably persist this information
        if serving_entity_ids == feature_list.primary_entity_ids:
            ...
            feature_service_name = "_".join(
                [feature_list.versioned_name] + [str(entity_id) for entity_id in serving_entity_ids]
            )
        df_feast_online_features = await self._get_online_features_feast(
            feast_store=feast_store,
            feast_service_name=feature_service_name,
            ...
            required_serving_names=[
                provided_entities_mapping[entity_id].serving_names[0]
    ...
    async def _get_online_features_feast(
        ...
        required_serving_names: List[str]
            Serving names that are provided in the request
        ...
        for offline_store_table_doc in offline_store_table_docs["data"]:
            ...
            # TODO: logic not quite right, should check using tables with primary_entity_ids
            #  matching the serving_entity_ids
