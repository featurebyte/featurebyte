...
def timestamp_to_tile_index(input_datetime: datetime, tile_spec: TileSpec) -> int:
    """
    ...
    adjusted_ts = input_datetime - timedelta(seconds=offset)
    period_in_seconds = (adjusted_ts - datetime(1970, 1, 1)).total_seconds()
...
def tile_index_to_timestamp(tile_index: int, tile_spec: TileSpec) -> datetime:
    ...
    epoch_ts = datetime.fromtimestamp(period_in_seconds).astimezone(pytz.timezone("UTC"))
