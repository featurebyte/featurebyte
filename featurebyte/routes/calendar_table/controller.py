"""
CalendarTable API route controller
"""
...
from featurebyte.schema.calendar_table import CalendarTableList, CalendarTableServiceUpdate
...
class CalendarTableController(
    BaseTableDocumentController[CalendarTableModel, CalendarTableService, CalendarTableList]
):
    """
    CalendarTable controller
    """

    paginated_document_class = CalendarTableList
    document_update_schema_class = CalendarTableServiceUpdate
    semantic_tag_rules = {
        **BaseTableDocumentController.semantic_tag_rules,
        "series_id_column": SemanticType.SERIES_ID,
        "calendar_datetime_column": SemanticType.CALENDAR_DATE,
    }

    def __init__(
        self,
        calendar_table_service: TableDocumentService,
        ...
        super().__init__(
            service=calendar_table_service,
    ...
    async def get_info(self, document_id: ObjectId, verbose: bool) -> CalendarTableInfo:
        """
        ...
        CalendarTableInfo
        """
        calendar_table = await self.service.get_document(document_id=document_id)
        table_dict = await self.table_info_service.get_table_info(
            data_document=calendar_table, verbose=verbose
        )
        return CalendarTableInfo(
            **table_dict,
            series_id_column=calendar_table.series_id_column,
            calendar_datetime_column=calendar_table.calendar_datetime_column,
            calendar_datetime_schema=calendar_table.calendar_datetime_schema,
        )
