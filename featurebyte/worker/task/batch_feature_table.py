...
class BatchFeatureTableTask(DataWarehouseMixin, BaseTask[BatchFeatureTableTaskPayload]):
    ...
    async def execute(self, payload: BatchFeatureTableTaskPayload) -> Any:
        ...
        batch_request_table_model = await self.batch_request_table_service.get_document(payload.batch_request_table_id)

        location = await self.batch_feature_table_service.generate_materialized_table_location(payload.feature_store_id)

        # retrieve feature list from deployment
        deployment: DeploymentModel = await self.deployment_service.get_document(document_id=payload.deployment_id)
        ...
        async with self.drop_table_on_error(
            ...
            (
                columns_info,
                num_rows,
            ) = await self.batch_request_table_service.get_columns_info_and_num_rows(db_session, location.table_details)
