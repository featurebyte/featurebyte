...
class EventDataMigrationService(EventTableService, MigrationServiceMixin):
    """EventDataMigrationService class"""

    @migrate(version=1, description="Rename collection name from event_table to table")
    async def change_collection_name_from_event_data_to_table_data(self) -> None:
        """Change collection name from event data to table"""
        collection_names = await self.persistent.list_collection_names()
        if "event_table" not in collection_names:
            # do nothing if the collection does not exist
            return

        await self.persistent.rename_collection(
            collection_name="event_table", new_collection_name="table"
        )
        ...
        _, total_before = await self.persistent.find(
            collection_name="table", query_filter=query_filter, page_size=page_size
        )
