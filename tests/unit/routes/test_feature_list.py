...
class TestFeatureListApi(BaseCatalogApiTestSuite):
    ...
    def test_update_200__naive_prediction(
        ...
        # update with valid naive_prediction (feature id)
        response = test_api_client.patch(
            f"{self.base_route}/{doc_id}", json={"naive_prediction": feature_id}
        )
        assert response.status_code == HTTPStatus.OK
        assert response.json()["naive_prediction"] == feature_id

        # update with invalid naive_prediction (nonexistent id)
        response = test_api_client.patch(
            f"{self.base_route}/{doc_id}", json={"naive_prediction": str(ObjectId())}
        )
        assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY
    ...
    def test_create_201__with_naive_prediction(self, test_api_client_persistent, user_id):
        ...
        # create feature list with naive_prediction (feature id)
        payload = self.payload.copy()
        payload["_id"] = str(ObjectId())
        payload["name"] = "fl_with_naive_pred"
        payload["naive_prediction"] = feature_id
    ...
    def test_create_422__with_invalid_naive_prediction(self, test_api_client_persistent, user_id):
        ...
        payload["naive_prediction"] = str(ObjectId())
        response = test_api_client.post(self.base_route, json=payload)
        assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY
