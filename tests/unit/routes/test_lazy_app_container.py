...
def test_disable_block_modification_check(app_container):
    ...
    with app_container.block_modification_checker.disable_block_modification_check():
        # level-1 service
        assert (
            app_container.event_table_service.block_modification_checker.block_modification is False
        )
        # level-2 service
        service = app_container.table_facade_service.event_table_service
        assert service.block_modification_checker.block_modification is False
        # level-3 service
        ...
        assert service.entity_service.block_modification_checker.block_modification is False

    # outside the context manager, the check should be enabled
    # level-1 service
    assert app_container.event_table_service.block_modification_checker.block_modification is True
    # level-2 service
    service = app_container.table_facade_service.event_table_service
    assert service.block_modification_checker.block_modification is True
    # level-3 service
    service = app_container.table_facade_service.table_columns_info_service.semantic_service
    assert service.block_modification_checker.block_modification is True
    # level-4 service
    ...
    assert service.entity_service.block_modification_checker.block_modification is True
