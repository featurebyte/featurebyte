...
async def test_data_document_services__retrieval(
    ...
    msg = f'EventTable (id: "{item_data.id}") not found. Please save the EventTable object first.'
    assert msg in str(exc.value)

    with pytest.raises(DocumentNotFoundError) as exc:
        await item_data_service.get_document(document_id=event_data.id)
    expected_msg = (
        f'ItemTable (id: "{event_data.id}") not found. Please save the ItemTable object first.'
    )
...
async def test_data_document_services__creation_conflict(
    ...
    with pytest.raises(DocumentConflictError) as exc:
        ...
        await event_data_service.create_document(data=EventTableCreate(**event_data_dict))

    expected_msg = (
        f'Table (id: "{item_data.id}") already exists. '
        f'Get the existing object by `ItemTable.get(name="{item_data.name}")`.'
    )
    assert expected_msg in str(exc.value)
    ...
    with pytest.raises(DocumentConflictError) as exc:
        ...
        await item_data_service.create_document(data=ItemTableCreate(**item_data_dict))

    expected_msg = (
        f'Table (id: "{event_data.id}") already exists. '
        f'Get the existing object by `EventTable.get(name="{event_data.name}")`.'
    )
    assert expected_msg in str(exc.value)
