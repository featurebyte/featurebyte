...
async def test_migration_v1__when_event_data_collection_not_exists(user, persistent):
    ...
    assert "event_table" not in collection_names_before
...
@pytest.mark.asyncio
@patch("featurebyte.service.base_table_document.FeatureStoreService")
async def test_migrate_all_records(
    ...
    for i in range(total_records):
        ...
        await event_data_migration_service.create_document(data=EventTableCreate(**payload))
