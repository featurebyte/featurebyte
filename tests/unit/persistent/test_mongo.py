...
async def test_insert_one(mongo_persistent, test_document, disable_audit):
    ...
    inserted_id = await persistent.insert_one(
        collection_name="data", document=test_document, user_id=user_id, disable_audit=disable_audit
    )
    ...
    hist_docs_generator = persistent.historical_document_generator(collection_name="data", document_id=inserted_id)
    ...
    await persistent._migrate_audit_records(collection_name="data", document_id=inserted_id, migrate_func=lambda d: d)
...
@pytest.mark.parametrize("disable_audit", [False, True])
@pytest.mark.asyncio
async def test_insert_many(mongo_persistent, test_documents, disable_audit):
    """
...
async def test_find_many(mongo_persistent, test_documents):
    ...
    # test pagination
    docs, total = await persistent.find(collection_name="data", query_filter={}, page_size=2, page=1)
    assert docs == test_documents[:2]
    assert total == 3
    docs, total = await persistent.find(collection_name="data", query_filter={}, page_size=2, page=2)
    assert docs == test_documents[2:]
    assert total == 3
    docs, total = await persistent.find(collection_name="data", query_filter={}, page_size=0, page=2)
    assert docs == test_documents
    assert total == 3
...
async def test_update_one(mongo_persistent, test_document, disable_audit):
    ...
    hist_docs_generator = persistent.historical_document_generator(collection_name="data", document_id=doc_id)
    hist_docs = [doc async for _, doc in hist_docs_generator]
    ...
    await persistent._migrate_audit_records(collection_name="data", document_id=doc_id, migrate_func=lambda d: d)
...
@pytest.mark.parametrize("disable_audit", [False, True])
@pytest.mark.asyncio
async def test_update_many(mongo_persistent, test_documents, disable_audit):
    """
    Test updating one document
    """
    user_id = ObjectId()
...
@pytest.mark.parametrize("disable_audit", [False, True])
@pytest.mark.asyncio
async def test_replace_one(mongo_persistent, test_document, test_documents, disable_audit):
    """
...
@pytest.mark.parametrize("disable_audit", [False, True])
@pytest.mark.asyncio
async def test_delete_one(mongo_persistent, test_documents, disable_audit):
    """
    ...
    result = await persistent.delete_one(
        user_id=user_id, collection_name="data", query_filter={}, disable_audit=disable_audit
    )
...
@pytest.mark.parametrize("disable_audit", [False, True])
@pytest.mark.asyncio
async def test_delete_many(mongo_persistent, test_documents, disable_audit):
    """
    ...
    persistent, client = mongo_persistent
    await client["test"]["data"].insert_many(test_documents)
    result = await persistent.delete_many(
        user_id=user_id, collection_name="data", query_filter={}, disable_audit=disable_audit
    )
...
async def test_get_audit_logs(mongo_persistent, test_document):
    ...
    # insert a doc
    inserted_id = await persistent.insert_one(collection_name="data", document=test_document, user_id=None)
    ...
    # delete the doc
    num_deleted = await persistent.delete_one(collection_name="data", query_filter={"_id": inserted_id}, user_id=None)
