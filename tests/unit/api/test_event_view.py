...
def test_from_event_data(snowflake_event_data, mock_api_object_cache):
    ...
    filtered_columns_info = [
        col_info for col_info in snowflake_event_data.columns_info if col_info.name != "created_at"
    ]
    assert event_view_first.columns_info == filtered_columns_info
    ...
    assert event_view_second.columns_info == filtered_columns_info
