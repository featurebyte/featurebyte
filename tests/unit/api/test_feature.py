...
def feature_with_clean_column_names_fixture(saved_event_data, cust_id_entity):
    ...
    feature = event_view.groupby("cust_id").aggregate_over(
        ...
        feature_job_setting={"frequency": "6h", "time_window": "3d"},
    )["sum_3d"]
...
def test_feature_definition(feature_with_clean_column_names):
    ...
    event_view = EventView.from_event_data(
        ...
        column_cleaning_operations=[
            ColumnCleaningOperation(
                column_name="col_float", cleaning_operations=[MissingValueImputation(imputed_value=0)]
            ),
            ColumnCleaningOperation(
                column_name="cust_id", cleaning_operations=[MissingValueImputation(imputed_value=0)]
            ),
        ],
    )
