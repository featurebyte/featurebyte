...
def test_create_use_case(catalog, float_target, context):
    ...
    assert retrieved_use_case.higher_prediction_is_better is True  # Default value
...
def test_create_use_case_with_higher_prediction_is_better_false(catalog, float_target, context):
    """
    Test UseCase.create method with higher_prediction_is_better=False (e.g., for churn targets)
    """
    _ = catalog
    ...
    use_case = UseCase.create(
        ...
        higher_prediction_is_better=False,
    )

    # Test get use case and verify higher_prediction_is_better is False
    retrieved_use_case = UseCase.get_by_id(use_case.id)
    assert retrieved_use_case.name == "test_use_case_churn"
    assert retrieved_use_case.higher_prediction_is_better is False

    # Test update_higher_prediction_is_better
    retrieved_use_case.update_higher_prediction_is_better(True)
    updated_use_case = UseCase.get_by_id(use_case.id)
    assert updated_use_case.higher_prediction_is_better is True
