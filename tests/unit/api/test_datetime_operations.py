...
def test_date_diff_with_varchar_timestamp(
    saved_event_table, cust_id_entity, arbitrary_default_feature_job_setting, mock_deployment_flow
):
    """
    ...
    expected = textwrap.dedent(
        ...
        def odfv_diff_in_day_v250101_[FEATURE_ID](
            ...
            request_time = pd.to_datetime(
                inputs["POINT_IN_TIME"], utc=True, format="%y-%m-%d %H:%M"
            )
            ...
            feat = pd.to_datetime(
                inputs["__diff_in_day_V250101__part0"],
                utc=True,
                format="%y-%m-%d %H:%M",
            ) - pd.to_datetime(request_col, utc=True)
