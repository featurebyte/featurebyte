...
def test_event_view_graph_node(event_table_data, event_input_node):
    ...
    assert set(graph_node.get_required_input_columns(input_index=0)) == {"amount", "event_id"}
    assert columns_info == [
        {
            "name": "event_id",
            "dtype": "INT",
...
def test_item_view_graph_node(item_table_data, event_table_data, item_input_node, event_input_node):
    ...
    assert set(item_graph_node.get_required_input_columns(input_index=0)) == {
        "item_name",
        "item_amount",
        "item_id",
    }
    assert set(item_graph_node.get_required_input_columns(input_index=1)) == {
        "amount",
        "event_id",
        "event_timestamp",
    }
