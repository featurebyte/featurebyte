...
def source_table_input_node_fixture(feature_store_details, source_table_data):
    ...
    assert input_node.dict() == {
        ...
        "parameters": {
            ...
            "id": None,
            "table_details": source_table_data.tabular_source.table_details.dict(),
            "type": "source_table",
        },
        "output_type": "frame",
    }
...
def event_input_node_fixture(feature_store_details, event_table_data):
    ...
    assert input_node.dict() == {
        ...
        "parameters": {
            ...
            "table_details": event_table_data.tabular_source.table_details.dict(),
            "id_column": "event_id",
...
def dimension_input_node_fixture(feature_store_details, dimension_table_data):
    ...
    assert input_node.dict() == {
        ...
        "parameters": {
            ...
            "table_details": dimension_table_data.tabular_source.table_details.dict(),
            "id_column": "user_id",
        ...
        SELECT
          ...
          CAST(CASE
            WHEN (
              CAST(CASE WHEN (
                "amount" IS NULL
              ) THEN 0.0 ELSE "amount" END AS DOUBLE) < 0.0
            )
            THEN NULL
            ELSE CAST(CASE WHEN (
              "amount" IS NULL
            ) THEN 0.0 ELSE "amount" END AS DOUBLE)
...
def test_event_view_graph_node(event_table_data, event_input_node):
    ...
    compare_pydantic_obj(columns_info, expected=expected_columns_info)
