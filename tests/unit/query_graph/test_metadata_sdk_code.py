...
def test_class_enum_and_object_class():
    ...
    event_data = ClassEnum.EVENT_TABLE(ClassEnum.OBJECT_ID(object_id), name="event_data")
    assert (
        str(event_data)
        == repr(event_data)
        == 'EventTable(ObjectId("63eaeafcbe3a62da29705ad1"), name="event_data")'
    )
    assert event_data.extract_import() == {ClassEnum.EVENT_TABLE.value, ClassEnum.OBJECT_ID.value}

    # check ObjectClass object inside containers (list & dict)
    event_data = ClassEnum.EVENT_TABLE(
    ...
    assert event_data.extract_import() == {
    ...
    # check _method_name
    event_data = ClassEnum.EVENT_TABLE(ClassEnum.OBJECT_ID("1234"), _method_name="get_by_id")
    assert str(event_data) == repr(event_data) == 'EventTable.get_by_id(ObjectId("1234"))'
    assert event_data.extract_import() == {ClassEnum.EVENT_TABLE.value, ClassEnum.OBJECT_ID.value}
...
def test_code_generator():
    ...
    code_gen.add_statements(
        statements=[(VariableNameStr("event_data"), ClassEnum.EVENT_TABLE(name="event_data"))],
    )
    assert code_gen.generate() == (
        "# Generated by SDK version: 0.1.0\n"
        "from featurebyte import EventTable\n\n"
        'event_data = EventTable(name="event_data")'
    )

    code_gen.add_statements(
        statements=[
            (VariableNameStr("event_data_1"), ClassEnum.EVENT_TABLE(name="another_event_data"))
        ],
    )
    assert code_gen.generate() == (
        "# Generated by SDK version: 0.1.0\n"
        "from featurebyte import EventTable\n\n"
        'event_data = EventTable(name="event_data")\n'
        'event_data_1 = EventTable(name="another_event_data")'
    )
