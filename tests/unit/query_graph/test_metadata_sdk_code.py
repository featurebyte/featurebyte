...
def test_class_enum_and_object_class():
    ...
    # check _method_name
    event_data = ClassEnum.EVENT_DATA(ClassEnum.OBJECT_ID("1234"), _method_name="get_by_id")
    assert str(event_data) == repr(event_data) == 'EventData.get_by_id(ObjectId("1234"))'
    assert event_data.extract_import() == {ClassEnum.EVENT_DATA.value, ClassEnum.OBJECT_ID.value}
...
def test_code_generator():
    ...
    code_gen.add_statements(
        statements=[(VariableNameStr("event_data"), ClassEnum.EVENT_DATA(name="event_data"))],
    )
    assert code_gen.generate() == (
        "# Generated by SDK version: 0.1.0\n"
        "from featurebyte import EventData\n\n"
        'event_data = EventData(name="event_data")'
    )

    code_gen.add_statements(
        statements=[
            (VariableNameStr("event_data_1"), ClassEnum.EVENT_DATA(name="another_event_data"))
        ],
    )
    assert code_gen.generate() == (
        "# Generated by SDK version: 0.1.0\n"
        "from featurebyte import EventData\n\n"
        'event_data = EventData(name="event_data")\n'
        'event_data_1 = EventData(name="another_event_data")'
    )
