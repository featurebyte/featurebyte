...
from featurebyte.enum import AggFunc, DBVarType, SourceType, TableDataType
...
def test_generic_function__view_type(global_graph, input_node, database_details):
    ...
    state = SDKCodeExtractor(graph=global_graph).extract(
        node=gfunc, database_details=database_details, source_type=SourceType.SNOWFLAKE
    )
    definition = state.code_generator.generate()
    expected_definition_lines = [
        'col_1 = udf_my_func(col, 1, 2.0, "hello", True)',
        "output = col_1",
    ]
...
def test_generic_function__feature_type(
    global_graph,
    query_graph_with_groupby_and_feature_nodes,
    ...
    definition = state.code_generator.generate()
    expected_definition_lines = ["feat_1 = udf_my_func(1, feat)", "output = feat_1"]
    assert definition.endswith("\n".join(expected_definition_lines))
