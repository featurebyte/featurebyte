...
def test_lookup_features_dimension_view(dimension_view, source_type):
    ...
    """
    target_name = "ItemTypeTargetDimensionView"
    dimension_target = dimension_view["item_type"].as_target(target_name, fill_value=None)
...
def test_event_view_lookup_features(event_table, transaction_data_upper_case, source_type):
    ...
    target = view[lookup_column_name].as_target(target_name, fill_value=None)
    check_lookup_feature_or_target_is_time_aware(
...
def test_scd_lookup_target_with_offset(config, scd_table, scd_dataframe, source_type):
    ...
    scd_lookup_target = scd_view["User Status"].as_target(
        target_name, offset=offset, fill_value=None
    )
...
def test_event_lookup_target_with_offset(
    ...
    event_target = view[lookup_column_name].as_target(target_name, offset=offset, fill_value=None)
    _target_with_offset_test_helper(
        event_target,
        target_name,
        lookup_column_name,
