...
async def test_schedule_update_feature_store__update_feature_value(
    ...
    new_sql_query = f"""
        select
          {entity_col_names},
          CAST('{feature_name}' AS VARCHAR) as {InternalName.ONLINE_STORE_RESULT_NAME_COLUMN},
...
async def test_schedule_update_feature_store__insert_with_new_feature_column(
    ...
    new_sql_query = f"""
        select
          {entity_col_names},
          CAST('{new_feature_name}' AS VARCHAR) as {InternalName.ONLINE_STORE_RESULT_NAME_COLUMN},
          {quote_feature_name} as {InternalName.ONLINE_STORE_VALUE_COLUMN}
        FROM (
