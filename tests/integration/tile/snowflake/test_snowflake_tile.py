...
def test_schedule_online_tile(snowflake_tile, snowflake_session):
    ...
    task_name = f"SHELL_TASK_{snowflake_tile.tile_spec.tile_id}_ONLINE".upper()
...
def test_schedule_offline_tile(snowflake_tile, snowflake_session):
    ...
    task_name = f"SHELL_TASK_{snowflake_tile.tile_spec.tile_id}_OFFLINE".upper()
...
def test_insert_tile_registry(snowflake_tile, snowflake_session):
    ...
    sql = f"SELECT * FROM TILE_REGISTRY WHERE TILE_ID = '{snowflake_tile.tile_spec.tile_id}'"
    result = snowflake_session.execute_query(sql)
    assert len(result) == 1
    assert result["TILE_ID"].iloc[0] == snowflake_tile.tile_spec.tile_id
    assert bool(result["IS_ENABLED"].iloc[0]) is True
    assert (
        result["TIME_MODULO_FREQUENCY_SECOND"].iloc[0]
        == snowflake_tile.tile_spec.time_modulo_frequency_second
    )
