...
def test_schedule_generate_tile_online(fb_db_session):
    ...
    sql = f"call SP_TILE_GENERATE_SCHEDULE('{table_name}', 183, 3, 5, 1440, '{tile_sql}', '{col_names}', 'ONLINE', {tile_monitor}, '{tile_end_ts}')"
...
def test_schedule_monitor_tile_online(fb_db_session):
    ...
    sql = f"call SP_TILE_GENERATE_SCHEDULE('{table_name}', 183, 3, 5, 1440, '{tile_sql}', '{col_names}', 'ONLINE', {tile_monitor}, '{tile_end_ts_2}')"
    fb_db_session.execute_query(sql)

    sql = f"SELECT COUNT(*) as TILE_COUNT FROM {table_name}_TILE_MONITOR"
    result = fb_db_session.execute_query(sql)
