WITH "SNAPSHOTS_REQUEST_TABLE_DISTINCT_POINT_IN_TIME_1ce40a759ab7017d" AS (
  SELECT
    "POINT_IN_TIME",
    "serving_cust_id",
    DATE_ADD(DATE_ADD(DATE_TRUNC('day', "POINT_IN_TIME"), -86400, 'SECOND'), -259200, 'SECOND') AS "__FB_SNAPSHOTS_ADJUSTED_POINT_IN_TIME"
  FROM (
    SELECT DISTINCT
      "POINT_IN_TIME",
      "serving_cust_id"
...
), "SNAPSHOTS_REQUEST_TABLE_DISTINCT_ADJUSTED_POINT_IN_TIME_1ce40a759ab7017d" AS (
  SELECT DISTINCT
    "__FB_SNAPSHOTS_ADJUSTED_POINT_IN_TIME",
    "serving_cust_id"
  FROM "SNAPSHOTS_REQUEST_TABLE_DISTINCT_POINT_IN_TIME_1ce40a759ab7017d"
)
SELECT
  POINT_IN_TIME,
  cust_id,
...
LEFT JOIN (
  ...
  FROM "SNAPSHOTS_REQUEST_TABLE_DISTINCT_POINT_IN_TIME_1ce40a759ab7017d" AS DISTINCT_POINT_IN_TIME
  LEFT JOIN (
    SELECT
