WITH "REQUEST_TABLE_TIME_SERIES_W3_MONTH_O1_MONTH_0 0 * * *_Etc/UTC_CUSTOMER_ID_DISTINCT_BY_POINT_IN_TIME" AS (
  SELECT DISTINCT
    "POINT_IN_TIME",
    "CUSTOMER_ID",
    "__FB_CRON_JOB_SCHEDULE_DATETIME"
  FROM "REQUEST_TABLE_0 0 * * *_Etc/UTC"
), "REQUEST_TABLE_TIME_SERIES_W3_MONTH_O1_MONTH_0 0 * * *_Etc/UTC_CUSTOMER_ID_DISTINCT_BY_SCHEDULED_JOB_TIME" AS (
  SELECT
    "CUSTOMER_ID",
    "__FB_CRON_JOB_SCHEDULE_DATETIME",
    (
...
LEFT JOIN (
  SELECT
    ...
    AGGREGATED."_fb_internal_CUSTOMER_ID_time_series_sum_a_cust_id_None_W3_MONTH_0 0 * * *_Etc/UTC_input_1"
  FROM "REQUEST_TABLE_TIME_SERIES_W3_MONTH_O1_MONTH_0 0 * * *_Etc/UTC_CUSTOMER_ID_DISTINCT_BY_POINT_IN_TIME" AS DISTINCT_POINT_IN_TIME
  LEFT JOIN (
    SELECT
    ...
    FROM (
      SELECT
        ...
        VIEW."a"
      FROM "REQUEST_TABLE_TIME_SERIES_W3_MONTH_O1_MONTH_0 0 * * *_Etc/UTC_CUSTOMER_ID_DISTINCT_BY_SCHEDULED_JOB_TIME" AS REQ
      INNER JOIN "VIEW_bf71d0f1326a235f" AS VIEW
