WITH "SNAPSHOTS_REQUEST_TABLE_DISTINCT_POINT_IN_TIME_9e3ea5d5a9e28504" AS (
  SELECT
    "POINT_IN_TIME",
    "serving_cust_id",
    DATE_ADD(
      DATE_TRUNC('day', "__FB_CRON_JOB_SCHEDULE_DATETIME_0 8 * * *_Asia/Singapore_None"),
      -86400,
      'SECOND'
  ...
  FROM (
    SELECT DISTINCT
      "POINT_IN_TIME",
      "__FB_CRON_JOB_SCHEDULE_DATETIME_0 8 * * *_Asia/Singapore_None",
      "serving_cust_id"
    FROM "REQUEST_TABLE"
  )
), "SNAPSHOTS_REQUEST_TABLE_DISTINCT_ADJUSTED_POINT_IN_TIME_9e3ea5d5a9e28504" AS (
  SELECT DISTINCT
    "__FB_SNAPSHOTS_ADJUSTED_POINT_IN_TIME",
    "serving_cust_id"
  FROM "SNAPSHOTS_REQUEST_TABLE_DISTINCT_POINT_IN_TIME_9e3ea5d5a9e28504"
)
SELECT
  POINT_IN_TIME,
  cust_id,
...
LEFT JOIN (
  ...
  FROM "SNAPSHOTS_REQUEST_TABLE_DISTINCT_POINT_IN_TIME_9e3ea5d5a9e28504" AS DISTINCT_POINT_IN_TIME
  LEFT JOIN (
    SELECT
    ...
    FROM "SNAPSHOTS_REQUEST_TABLE_DISTINCT_ADJUSTED_POINT_IN_TIME_9e3ea5d5a9e28504" AS REQ
    INNER JOIN (
      SELECT
        *
