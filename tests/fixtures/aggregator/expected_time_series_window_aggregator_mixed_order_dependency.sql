WITH "REQUEST_TABLE_TIME_SERIES_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_CUSTOMER_ID_DISTINCT_BY_POINT_IN_TIME" AS (
...
), "VIEW_b97a3d9de4e667c8_DISTINCT_REFERENCE_DATETIME" AS (
  SELECT
  ...
  FROM (
    SELECT DISTINCT
      "snapshot_date"
    FROM "VIEW_b97a3d9de4e667c8"
    WHERE
      NOT "snapshot_date" IS NULL
  )
), "VIEW_ec950da945e163b9_DISTINCT_REFERENCE_DATETIME" AS (
  SELECT
    "snapshot_date" AS "__FB_VIEW_REFERENCE_DATETIME",
    DATE_PART(EPOCH_SECOND, TO_TIMESTAMP("snapshot_date", 'YYYYMMDD')) AS "__FB_VIEW_TIMESTAMP_EPOCH"
  FROM (
...
SELECT
  POINT_IN_TIME,
  cust_id,
  "T0"."_fb_internal_CUSTOMER_ID_time_series_latest_a_cust_id_None_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_input_1" AS "_fb_internal_CUSTOMER_ID_time_series_latest_a_cust_id_None_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_input_1",
  "T1"."_fb_internal_CUSTOMER_ID_time_series_sum_a_cust_id_None_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_input_1" AS "_fb_internal_CUSTOMER_ID_time_series_sum_a_cust_id_None_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_input_1"
FROM REQUEST_TABLE
LEFT JOIN (
  SELECT
    DISTINCT_POINT_IN_TIME."POINT_IN_TIME",
    DISTINCT_POINT_IN_TIME."CUSTOMER_ID",
    AGGREGATED."_fb_internal_CUSTOMER_ID_time_series_latest_a_cust_id_None_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_input_1"
  ...
  LEFT JOIN (
    SELECT
      "__FB_CRON_JOB_SCHEDULE_DATETIME",
      "CUSTOMER_ID",
      "_fb_internal_CUSTOMER_ID_time_series_latest_a_cust_id_None_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_input_1"
    FROM (
      SELECT
        ROW_NUMBER() OVER (PARTITION BY "__FB_CRON_JOB_SCHEDULE_DATETIME", "CUSTOMER_ID" ORDER BY "__FB_VIEW_TIMESTAMP_EPOCH" DESC) AS "__FB_GROUPBY_HELPER_ROW_NUMBER",
        "__FB_CRON_JOB_SCHEDULE_DATETIME" AS "__FB_CRON_JOB_SCHEDULE_DATETIME",
        "CUSTOMER_ID" AS "CUSTOMER_ID",
        FIRST_VALUE("a") OVER (PARTITION BY "__FB_CRON_JOB_SCHEDULE_DATETIME", "CUSTOMER_ID" ORDER BY "__FB_VIEW_TIMESTAMP_EPOCH" DESC) AS "_fb_internal_CUSTOMER_ID_time_series_latest_a_cust_id_None_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_input_1"
      FROM (
        SELECT
        ...
        FROM (
          ...
          INNER JOIN "VIEW_b97a3d9de4e667c8_DISTINCT_REFERENCE_DATETIME" AS BUCKETED_REFERENCE_DATETIME
        ...
        INNER JOIN "VIEW_b97a3d9de4e667c8" AS VIEW
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          BUCKETED_REFERENCE_DATETIME."__FB_VIEW_TIMESTAMP_EPOCH"
        FROM "REQUEST_TABLE_TIME_SERIES_W7_DAY_0 0 * * *_Etc/UTC_Asia/Singapore_CUSTOMER_ID_DISTINCT_BY_SCHEDULED_JOB_TIME" AS REQ
        INNER JOIN "VIEW_ec950da945e163b9_DISTINCT_REFERENCE_DATETIME" AS BUCKETED_REFERENCE_DATETIME
          ON FLOOR(REQ."__FB_WINDOW_END_EPOCH" / 604800) = FLOOR(BUCKETED_REFERENCE_DATETIME."__FB_VIEW_TIMESTAMP_EPOCH" / 604800)
        WHERE
          BUCKETED_REFERENCE_DATETIME."__FB_VIEW_TIMESTAMP_EPOCH" >= REQ."__FB_WINDOW_START_EPOCH"
          AND BUCKETED_REFERENCE_DATETIME."__FB_VIEW_TIMESTAMP_EPOCH" < REQ."__FB_WINDOW_END_EPOCH"
      ...
      INNER JOIN "VIEW_ec950da945e163b9" AS VIEW
        ON RANGE_JOINED."__FB_VIEW_REFERENCE_DATETIME" = VIEW."snapshot_date"
        AND RANGE_JOINED."CUSTOMER_ID" = VIEW."cust_id"
    )
    GROUP BY
