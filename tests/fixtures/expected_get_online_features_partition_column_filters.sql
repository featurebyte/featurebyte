...
WITH ONLINE_REQUEST_TABLE AS (
  ...
  FROM (
    ...
    WHERE
      "date" >= TO_CHAR(CAST('2023-06-24 12:00:00' AS TIMESTAMP), 'YYYY-MM-DD HH24:MI:SS')
      AND "date" <= TO_CHAR(CAST('2023-10-08 12:00:00' AS TIMESTAMP), 'YYYY-MM-DD HH24:MI:SS')
  )
), "VIEW_9ca290447534b3be_DISTINCT_REFERENCE_DATETIME" AS (
  SELECT
    ...
    LEFT JOIN (
      ...
      FROM (
        ...
        FROM (
          ...
          INNER JOIN "VIEW_9ca290447534b3be_DISTINCT_REFERENCE_DATETIME" AS BUCKETED_REFERENCE_DATETIME
        ...
        INNER JOIN "VIEW_9ca290447534b3be" AS VIEW
