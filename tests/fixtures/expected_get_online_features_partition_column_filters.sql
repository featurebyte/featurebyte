...
WITH ONLINE_REQUEST_TABLE AS (
  ...
  FROM (
    ...
    WHERE
      "date" >= TO_CHAR(CAST('2023-06-01 12:00:00' AS TIMESTAMP), 'YYYY-MM-DD HH24:MI:SS')
      AND "date" <= TO_CHAR(CAST('2023-11-01 12:00:00' AS TIMESTAMP), 'YYYY-MM-DD HH24:MI:SS')
  )
), "VIEW_6222942afb71ee03_DISTINCT_REFERENCE_DATETIME" AS (
  SELECT
    ...
    LEFT JOIN (
      ...
      FROM (
        ...
        FROM (
          ...
          INNER JOIN "VIEW_6222942afb71ee03_DISTINCT_REFERENCE_DATETIME" AS BUCKETED_REFERENCE_DATETIME
        ...
        INNER JOIN "VIEW_6222942afb71ee03" AS VIEW
