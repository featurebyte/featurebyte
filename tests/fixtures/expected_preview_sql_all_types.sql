  ...
  LEFT JOIN (
    ...
    FROM (
      ...
      FROM (
        SELECT
          *,
          FLOOR(__FB_LAST_TILE_INDEX / 2160) AS __FB_LAST_TILE_INDEX_DIV_NUM_TILES
        FROM "REQUEST_TABLE_W7776000_F3600_BS900_M1800_CUSTOMER_ID"
      ) AS REQ
      INNER JOIN (
        SELECT
          *,
          FLOOR(INDEX / 2160) AS __FB_TILE_INDEX_DIV_NUM_TILES
        FROM TILE_F3600_M1800_B900_8502F6BC497F17F84385ABE4346FD392F2F56725
      ) AS TILE
        ON (
          REQ.__FB_LAST_TILE_INDEX_DIV_NUM_TILES = __FB_TILE_INDEX_DIV_NUM_TILES
