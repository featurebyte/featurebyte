...
SELECT * FROM (
  ...
  INNER JOIN (
    ...
    WHERE
      `event_timestamp` >= DATEADD(MONTH, -1, DATEADD(MINUTE, -30, CAST('2025-06-19 03:00:00' AS TIMESTAMP)))
      AND `event_timestamp` <= DATEADD(MONTH, 1, CAST('2025-06-19 03:00:00' AS TIMESTAMP))
  ) AS R
...
ALTER TABLE `db`.`schema`.`batch_prediction_table` ADD COLUMN `POINT_IN_TIME` TIMESTAMP;

MERGE WITH SCHEMA EVOLUTION INTO `db`.`schema`.`batch_prediction_table` AS `T`
USING (
  SELECT
...
ON `T`.`POINT_IN_TIME` = `S`.`POINT_IN_TIME`
WHEN NOT MATCHED THEN INSERT *;

ALTER TABLE `db`.`schema`.`batch_prediction_table` ALTER COLUMN `snapshot_date` SET NOT NULL;

ALTER TABLE `db`.`schema`.`batch_prediction_table` ALTER COLUMN `cust_id` SET NOT NULL;

ALTER TABLE `db`.`schema`.`batch_prediction_table` ADD CONSTRAINT `pk_batch_prediction_table`
