    ...
    SELECT
      ...
      OBJECT_AGG(
        CASE
          ...
          THEN '__MISSING__'
          ELSE CAST(INNER_."col_int" AS TEXT)
        END,
        TO_VARIANT(
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          "col_int",
          "_fb_internal_cust_id_window_w1800_count_e707f2276dfd37d2ca45a7ffc8ca6569d4c8a2a0_inner",
          ROW_NUMBER() OVER (PARTITION BY "cust_id" ORDER BY "_fb_internal_cust_id_window_w1800_count_e707f2276dfd37d2ca45a7ffc8ca6569d4c8a2a0_inner" DESC NULLS LAST) AS "__fb_object_agg_row_number"
        FROM (
          SELECT
            "cust_id" AS "cust_id",
            "col_int" AS "col_int",
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          "col_int",
          "_fb_internal_cust_id_window_w86400_count_e707f2276dfd37d2ca45a7ffc8ca6569d4c8a2a0_inner",
          ROW_NUMBER() OVER (PARTITION BY "cust_id" ORDER BY "_fb_internal_cust_id_window_w86400_count_e707f2276dfd37d2ca45a7ffc8ca6569d4c8a2a0_inner" DESC NULLS LAST) AS "__fb_object_agg_row_number"
        FROM (
          SELECT
            "cust_id" AS "cust_id",
            "col_int" AS "col_int",
