...
SELECT * FROM (
  ...
  FROM (
    ...
    WHERE
      ...
      AND (
        "event_timestamp" >= DATEADD(DAY, -7, CAST('2022-05-15T05:45:00' AS TIMESTAMP))
        AND "event_timestamp" <= DATEADD(DAY, 7, CAST('2022-05-15T06:45:00' AS TIMESTAMP))
      )
  )
  WHERE
    "event_timestamp" >= CAST('2022-05-15T05:45:00' AS TIMESTAMP)
...
SELECT * FROM (
  ...
  FROM (
    ...
    WHERE
      ...
      AND (
        "event_timestamp" >= DATEADD(DAY, -7, CAST('2022-05-15T08:45:00' AS TIMESTAMP))
        AND "event_timestamp" <= DATEADD(DAY, 7, CAST('2022-05-15T09:45:00' AS TIMESTAMP))
      )
  )
  WHERE
    "event_timestamp" >= CAST('2022-05-15T08:45:00' AS TIMESTAMP)
...
ALTER TABLE __FB_DEPLOYED_TILE_TABLE_000000000000000000000000 ADD COLUMN value_count_704bc9a2e9fe7b08d6c064fbacd6b3fcb0185da9 FLOAT;
            ...
            merge into __FB_DEPLOYED_TILE_TABLE_000000000000000000000000 a using __TEMP_TILE_TABLE_000000000000000000000002 b
                on a.INDEX = b.INDEX AND EQUAL_NULL(a."cust_id", b."cust_id")
                when matched then
                    update set a.created_at = current_timestamp(), a.value_count_704bc9a2e9fe7b08d6c064fbacd6b3fcb0185da9 = b.value_count_704bc9a2e9fe7b08d6c064fbacd6b3fcb0185da9
                when not matched then
